...
LAST_CHECKPOINT_PATH = MODEL_DIR / "custom_cnn_last_checkpoint.pth"
BEST_MODEL_PATH = MODEL_DIR / "custom_cnn_best.pth"
FINAL_MODEL_PATH = MODEL_DIR / "custom_cnn_final.pth"

HISTORY_CSV_PATH = RESULTS_DIR / "custom_cnn_history.csv"
RESULTS_JSON_PATH = RESULTS_DIR / "custom_cnn_results.json"
RESULTS_TEXT_PATH = RESULTS_DIR / "custom_cnn_results.txt"

ACCURACY_CHART_PATH = RESULTS_DIR / "custom_cnn_accuracy.png"
LOSS_CHART_PATH = RESULTS_DIR / "custom_cnn_loss.png"
...
# Reduce the learning rate after this many unimproved epochs.
LR_PATIENCE = 2

# Set to True to restart training from epoch 1.
# Leave False to resume automatically from a saved checkpoint.
...
required_directories = [TRAIN_DIR, VAL_DIR, TEST_DIR]

for directory in required_directories:
    if not directory.exists():
        raise FileNotFoundError(
            f"Required folder not found: {directory}\n"
            "Run the dataset preprocessing and splitting scripts first."
        )
...
# Moderate augmentation, matching your project methodology.
...
if number_of_classes != 100:
    print(
        f"Warning: expected 100 classes, "
...
# num_workers=0 is the safest setting for Windows and low-memory computers.
...
class CustomCNN(nn.Module):
    def __init__(self, number_of_output_classes: int):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.40),
            nn.Linear(128, number_of_output_classes)
        )
...
model = CustomCNN(number_of_classes).to(device)
...
def load_checkpoint():
    ...
    if START_FROM_SCRATCH:
        print("Starting from scratch as requested.")
        return
...
def save_training_charts():
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(ACCURACY_CHART_PATH, dpi=200)
    plt.close()
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(LOSS_CHART_PATH, dpi=200)
    plt.close()
...
def train_one_epoch():
    ...
    for images, labels in train_loader:
        ...
        # Explicitly discard references to large tensors.
...
def evaluate_test_set():
    ...
    with torch.inference_mode():
        for images, labels in test_loader:
            ...
            top_5_correct += (
                top_5_predictions == labels.unsqueeze(1)
            ).any(dim=1).sum().item()
    ...
    top_1_accuracy = top_1_correct / total_examples
    top_5_accuracy = top_5_correct / total_examples

    average_inference_ms = (
        evaluation_seconds / total_examples
    ) * 1000
...
try:
    for epoch_index in range(start_epoch, MAX_EPOCHS):
        ...
        if improved:
            ...
            save_checkpoint(
                ...
                current_best_accuracy=best_val_accuracy,
                unimproved_epochs=epochs_without_improvement,
        ...
        # Save the latest state after every completed epoch.
        save_checkpoint(
        ...
        if (
            ...
            print(
                "\nEarly stopping activated because validation "
                "accuracy did not improve."
            )
            break

except KeyboardInterrupt:
    print("\nTraining was stopped manually.")
    ...
    print(
        "Progress was saved. Run this script again to resume."
    )
    ...
    print(
        "The latest available progress was saved. "
...
final_results = {
    ...
    "top_1_accuracy": test_results["top_1_accuracy"],
    "top_5_accuracy": test_results["top_5_accuracy"],
    "average_inference_ms_per_image":
        test_results["average_inference_ms_per_image"],
...
with RESULTS_TEXT_PATH.open(
    ...
    text_file.write("CUSTOM CNN FINAL RESULTS\n")
    text_file.write("=" * 40 + "\n")
    text_file.write(
